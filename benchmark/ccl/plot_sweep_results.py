...
def plot_results(data, args):
    ...
    for idx, operation in enumerate(operations):
        ...
        y_min = min(iris_bw.min(), rccl_bw[rccl_bw != None].min() if any(x is not None for x in rccl_bw) else iris_bw.min())
        y_max = max(iris_bw.max(), rccl_bw[rccl_bw != None].max() if any(x is not None for x in rccl_bw) else iris_bw.max())
        y_range = y_max - y_min
