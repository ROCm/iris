...
def all_gather_gemm_push(state, ctx):
    ...
    def _exec():
        ...
        push_shards_kernel[push_grid](
            A_local, A_inbox, signal_flags,
            M, K_local,
            A_local.stride(0), A_local.stride(1),
            A_inbox.stride(0), A_inbox.stride(1), A_inbox.stride(2),
            signal_flags.stride(0), signal_flags.stride(1),
            signal_flags.stride(2), signal_flags.stride(3),
            BLK_M, BLK_K, rank, world_size, heap_bases,
        )
