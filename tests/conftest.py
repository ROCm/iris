...
@pytest.fixture(params=["torch.float16", "torch.float32", "torch.int32", "torch.int64"])
def dtype(request):
...
@pytest.fixture(
    params=["iris.Semantic.ACQUIRE", "iris.Semantic.RELEASE", "iris.Semantic.ACQ_REL", "iris.Semantic.RELAXED"]
)
def sem(request):
...
@pytest.fixture(params=["iris.Scope.GPU", "iris.Scope.SYSTEM"])
def scope(request):
