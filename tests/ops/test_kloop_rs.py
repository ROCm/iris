#!/usr/bin/env python3
"""Test + benchmark simplest fused GEMM+RS: GEMM → iris.atomic_add to owner."""
...
# Output must be in symmetric heap for iris.atomic_add from peers
C_out = shmem.zeros((M_local, N), dtype=dtype)

if rank == 0:
    print(f"K-loop push RS: M={M}, N={N}, K={K_global}, TP={world_size}")
...
# Test
shmem.barrier()
matmul_reduce_scatter_kloop(shmem, C_out, A, B)
torch.cuda.synchronize()
...
if rank == 0:
    print(f"\nRCCL: {rccl_ms:.3f}ms")
    print("K-loop push sweep:")

for num_sms in [64, 128, 196, 256, 304]:
    for bm in [64, 128]:
        ...
        try:
            for _ in range(warmup // 2):
                C_out.zero_()
                shmem.barrier()
                matmul_reduce_scatter_kloop(shmem, C_out, A, B, block_m=bm, num_sms=num_sms)
            torch.cuda.synchronize()
            ...
            if rank == 0:
                print(f"  bm={bm} sms={num_sms:3d}: ERROR ({ex})")
