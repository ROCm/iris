...
def test_atomic_or_api(dtype, sem, scope, BLOCK_SIZE, num_ranks):
    # TODO: Adjust heap size.
    """Test with distributed setup."""
    
    @distributed_test(num_ranks=num_ranks)
    def _test_atomic_or_api_distributed(local_rank, world_size):
    shmem = iris.iris(1 << 20)
        
        return True
    
    # Run the distributed test
    result = _test_atomic_or_api_distributed()
