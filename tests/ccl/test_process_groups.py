...
def test_group_info_extraction():
    ...
    assert rank_in_group == rank_global == rank, f"group=None: rank mismatch"
    assert ws == world_size, f"group=None: world_size mismatch"
    assert rank_start == 0, f"group=None: rank_start should be 0"
    assert rank_stride == 1, f"group=None: rank_stride should be 1"
    ...
    if rank < 2:
        ...
        assert rank_in_group == rank, f"Consecutive group: rank_in_group mismatch"
        assert rank_global == rank, f"Consecutive group: rank_global mismatch"
        assert ws == 2, f"Consecutive group: world_size should be 2"
        assert rank_start == 0, f"Consecutive group: rank_start should be 0"
        assert rank_stride == 1, f"Consecutive group: rank_stride should be 1"
    ...
    if world_size >= 4:
        ...
        if rank in [0, 2]:
            ...
            assert rank_in_group == expected_rank_in_group, f"Strided group: rank_in_group should be {expected_rank_in_group}, got {rank_in_group}"
            assert rank_global == rank, f"Strided group: rank_global should be {rank}, got {rank_global}"
            assert ws == 2, f"Strided group: world_size should be 2"
            assert rank_start == 0, f"Strided group: rank_start should be 0"
            assert rank_stride == 2, f"Strided group: rank_stride should be 2"
...
def test_all_reduce_group_correctness():
    ...
    try:
        assert abs(actual_sum - expected_sum) < 1e-5, (
            f"Rank {rank} (group ranks={group_ranks}): "
