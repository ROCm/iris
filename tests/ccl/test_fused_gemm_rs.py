...
def test_fused_gemm_rs_workspace_reuse(dtype):
    ...
    workspace = shmem.ccl.gemm_reduce_scatter_preamble(iris_input, iris_weight)
    shmem.barrier()

    # Call 1
    out1 = shmem.ccl.gemm_reduce_scatter(iris_input, iris_weight, workspace=workspace)
    torch.cuda.synchronize()
...
def test_fused_gemm_rs_single_rank():
    ...
    iris_input = shmem.zeros((tokens, H), dtype=dtype)
    iris_input.copy_(input_tensor)

    shmem.barrier()
    iris_output = shmem.ccl.gemm_reduce_scatter(iris_input, weight)
    torch.cuda.synchronize()
...
def test_fused_gemm_rs_shape_validation():
    ...
    iris_input = shmem.zeros((32, 128), dtype=dtype)
    ...
    with pytest.raises(ValueError, match="inner dimension mismatch"):
        shmem.ccl.gemm_reduce_scatter(iris_input, bad_weight)

    # K not divisible by world_size
    if world_size > 1:
        iris_input2 = shmem.zeros((32, 128), dtype=dtype)
        ...
        with pytest.raises(ValueError, match="divisible by world_size"):
            shmem.ccl.gemm_reduce_scatter(iris_input2, bad_weight2)
