...
"""Test for examples/27_ccl_reduce_scatter/example.py."""
...
def test_ccl_reduce_scatter_example():
    if not dist.is_initialized():
        pytest.skip("torch.distributed not initialized")

    shmem = iris.iris(heap_size=2**31)
    try:
        _load_example("../../examples/27_ccl_reduce_scatter/example.py").run(shmem)
    finally:
        shmem.barrier()
        del shmem
        gc.collect()
