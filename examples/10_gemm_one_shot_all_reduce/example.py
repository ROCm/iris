...
"""
Example: GEMM One-Shot All-Reduce (K-split)

Demonstrates: Each rank computes a partial-K GEMM, stores the result locally,
signals completion to all other ranks via iris.atomic_add, then the completing
rank reads partial results from all ranks and reduces them into the global output.
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="GEMM One-Shot All-Reduce example.",
...
def main():
    ...
    # One-shot all-reduce needs extra buffers
    tile_completed = ctx.zeros((total_tiles,), device="cuda", dtype=torch.int32)
    locks = ctx.zeros((gemm_sms,), device="cuda", dtype=torch.int32)
    P = ctx.zeros(
        (gemm_sms, args["BLK_M"] * args["BLK_N"]),
        device="cuda",
        dtype=torch.float32,
    )
