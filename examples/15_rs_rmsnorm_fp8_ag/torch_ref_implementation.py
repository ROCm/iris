...
def quantize_fp16_to_fp8(input_tensor: torch.Tensor, scale: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor]:
    if scale is None:
...
def test_post_quantization_allgather():
    ...
    if max_diff < tolerance:
        print(f"✅ SUCCESS: Post-quantization All-Gather works!")
        return True
    else:
        print(f"❌ FAILURE: Results differ significantly")
        print(f"❌ RMSNorm on partial tensors is NOT equivalent to full tensor RMSNorm")
        return False
...
def main():
    ...
    if not success:
        print(f"\n❌ CONCLUSION:")
        print(f"   You CANNOT do All-Gather after RMSNorm and quantization.")
        print(f"   RMSNorm must operate on the FULL tensor.")
        print(f"   The correct pipeline is:")
        print(f"   Reduce-Scatter → All-Gather → RMSNorm → Quantization")
        
    else:
        print(f"\n✅ CONCLUSION:")
        print(f"   Post-quantization All-Gather works!")
        print(f"   This would be more efficient for communication.")
