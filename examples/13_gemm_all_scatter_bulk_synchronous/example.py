...
"""
Example: GEMM All-Scatter Bulk Synchronous (N-split)

Demonstrates: GEMM kernel runs first, computing full A x partial B into C.
Then a separate scatter kernel runs on the same stream, reading completed
tiles from C and writing them to all remote ranks via iris.store. The two
kernels run sequentially (bulk synchronous).
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="GEMM All-Scatter Bulk Synchronous example.",
...
def main():
    ...
    gemm_sms = next_pow2
    comm_sms = next_pow2  # Bulk synchronous uses same SM count for comm
