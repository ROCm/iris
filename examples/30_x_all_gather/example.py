...
"""
Example: iris.x all-gather

Demonstrates: iris.x.Tile, iris.x.make_tensor_view, iris.x.all_gather

Each rank fills an (M, N) tensor with (rank + 1), then all-gathers into a shared
output tensor of shape (M * world_size, N). After the operation, output rows
[r*M : (r+1)*M] contain the value (r + 1) for each rank r.
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="iris.x all-gather example",
...
def all_gather_kernel(
    ...
    for tile_id in range(pid, total_tiles, NUM_SMS):
        ...
        # Wrap as iris.x Tile and scatter to all ranks
        tile = iris.x.Tile(pid_m, pid_n, BLOCK_M, BLOCK_N, data)
        dst_view = iris.x.make_tensor_view(output_ptr, M * WORLD_SIZE, N, stride_out_m, stride_out_n)
        iris.x.all_gather(tile, dst_view, dim=0, ctx=dev_ctx)
