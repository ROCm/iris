...
Fused GEMM + reduce-scatter: each rank computes A @ B, then the result
is reduced (summed) and scattered along the M dimension so that each rank
receives a (M // world_size, N) slice.
...
def parse_args():
    ...
    parser.add_argument("-m", type=int, default=512, help="Rows of A (must be divisible by world_size)")
...
def main():
    ...
    output = ctx.zeros((M // world_size, N), dtype=dtype)

    ctx.barrier()
    ctx.ops.matmul_reduce_scatter(output, A, B)
    torch.cuda.synchronize()

    if rank == 0:
        ctx.info(
            f"matmul_reduce_scatter: world_size={world_size}, "
            f"A=({M},{K}), B=({K},{N}), output=({M // world_size},{N}), dtype={dtype}"
        )

    if args["validate"]:
        # Each rank computes the same GEMM; reduce-scatter sums and slices
        ref_full = torch.matmul(A.clone().float(), B.clone().float()).to(dtype) * world_size
        ref_slice = ref_full[rank * (M // world_size) : (rank + 1) * (M // world_size), :]
        assert torch.allclose(output.float(), ref_slice.float(), atol=1.0, rtol=0.05), (
            f"Rank {rank}: mismatch. Max diff: {(output.float() - ref_slice.float()).abs().max().item():.4f}"
        )
        if rank == 0:
