...
"""
Example: Independent GEMM + One-Shot All-Reduce (K-split)

Demonstrates: Each rank computes a partial-K GEMM independently, then a
separate all-reduce kernel reads partial results from all ranks, sums them,
and scatters the reduced output to every rank. GEMM and all-reduce run as
independent kernels. Optionally reads configs from a CSV file.
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Independent GEMM + One-Shot All-Reduce example.",
    ...
    parser.add_argument("--comm_sms", type=int, default=48, help="SMs for all-reduce kernel")
    parser.add_argument("--heap_size", type=int, default=1 << 33, help="Iris heap size")
    parser.add_argument("--csv", type=str, default=None, help="CSV file with per-run configs")
    parser.add_argument("--only_gemm", action="store_true", help="Run only GEMM (skip all-reduce)")
    parser.add_argument("--only_comm", action="store_true", help="Run only all-reduce (skip GEMM)")
    return vars(parser.parse_args())


def run_config(ctx, args, rank, world_size, cu_count):
    """Run one GEMM + all-reduce iteration with the given config."""
    ...
    local_A = A[:, rank * local_k : (rank + 1) * local_k]
    local_B = B[rank * local_k : (rank + 1) * local_k, :]
        ...
        with torch.cuda.stream(main_stream):
            ...
            if not only_gemm:
                ...
                all_reduce_kernel.run(
                    ...
                    rank,
                    world_size,
                    DISTRIBUTION=0,  # striding distribution
                )
    ...
    if args.get("validate") and not only_gemm and not only_comm:
        expected = torch.matmul(A.float(), B.float()).to(datatype)
        if torch.allclose(global_C, expected, atol=2.0):
            if rank == 0:
                print(f"Validation PASSED (M={M}, N={N}, K={K})")
        else:
            max_diff = (global_C.float() - expected.float()).abs().max().item()
