...
def gemm_reduce_scatter_preamble(
    ...
    if workspace.output_f32 is None or workspace.output_f32.shape != needed_shape:
        workspace.output_f32 = ctx.zeros(needed_shape, dtype=torch.float32)
...
def gemm_reduce_scatter(
    ...
    rank_in_group, rank_global, world_size, rank_start, rank_stride_val = extract_group_info(group, ctx)
    ...
    if workspace is None or workspace.output_f32 is None or workspace.output_f32.shape != (tokens, shard_size):
        workspace = gemm_reduce_scatter_preamble(input_tensor, weight_shard, ctx, config=config, workspace=workspace)
