...
class Config:
    ...
    def __post_init__(self):
        ...
        if self.all_reduce_variant not in ["atomic", "ring", "two_shot", "one_shot", "spinlock", "one_shot_fused"]:
            raise ValueError(
                f"all_reduce_variant must be one of: 'atomic', 'ring', 'two_shot', 'one_shot', 'spinlock', 'one_shot_fused', got {self.all_reduce_variant}"
            )
        ...
        if self.reduce_scatter_variant not in ("two_shot", "fused"):
            raise ValueError(
                f"reduce_scatter_variant must be 'two_shot' or 'fused', got '{self.reduce_scatter_variant}'"
            )
