...
class Config:
    ...
    def __post_init__(self):
        ...
        if self.all_reduce_variant not in ["atomic", "ring", "two_shot", "one_shot", "spinlock", "ll", "ll128"]:
            raise ValueError(
                f"all_reduce_variant must be one of: 'atomic', 'ring', 'two_shot', 'one_shot', 'spinlock', 'll', 'll128', got {self.all_reduce_variant}"
            )
