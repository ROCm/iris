...
_NCCL_LARGE_BYTES = 768 * 1024  # <768KB: native one_shot_fused, >=768KB: NCCL
...
def all_reduce(output_tensor, input_tensor, ctx, op=None, group=None, async_op=False, config=None, workspace=None):
    ...
    if config is None:
        config = Config(block_size_m=32, block_size_n=64, all_reduce_variant="one_shot_fused", comm_sms=64)
    if config.use_gluon:
        raise ValueError(
