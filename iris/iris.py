...
class Iris:
    ...
    class CCL:
        ...
        def all_reduce_rmsnorm(self, partial, residual, weight, eps=1e-6, group=None, async_op=False, config=None, workspace=None):
            ...
            Example:
                >>> ctx = iris.iris()
                >>> norm_out = ctx.ccl.all_reduce_rmsnorm(partial, residual, weight)
            """
            from iris.ccl.fused_ar_rmsnorm import all_reduce_rmsnorm as _all_reduce_rmsnorm

            return _all_reduce_rmsnorm(
                partial,
                residual,
                weight,
                self._iris,
