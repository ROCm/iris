...
class Iris:
    ...
    def put(
        ...
        wait_val = 0 if wait_ptr == 0 else int(wait_value if wait_value is not None else 0)
        signal_val = 0 if signal_ptr == 0 else int(signal_value)
...
def atomic_cas(
    ...
    if not USE_COPY_ENGINE:
        ...
        packet_offset_bytes = base + offset
        # Place command packet
        sdma_utils.place_atomic_cas_packet(
            queue_ptr_u32,
            packet_offset_bytes,
            dst_ptr_val,
            cmp,
