...
- TritonContext / GluonContext: Device-side contexts in iris.context
...
Example (TritonContext API):
    ...
    >>>     device_ctx = TritonContext.initialize(context_tensor, rank, world_size)
...
from .tracing import Tracing, TraceEvent  # noqa: F401  re-export for iris.TraceEvent
from .context.triton import TritonContext, TritonDeviceTracing, _translate_ptr  # noqa: F401
...
class Iris:
    ...
    def _build_device_context(self):
        ...
        if self.tracing.enabled:
            # Explicit buffer ordering (must match TritonContext.initialize extraction order)
            trace_buffer_ptrs = [
    ...
    def get_device_context(self):
        """
        Get the device context tensor for TritonContext initialization.
        ...
        Example:
            >>> import iris
            >>> from iris import TritonContext
            >>> import triton
            >>> import triton.language as tl
            >>>
            >>> ctx = iris.iris()
...
def load(
    ...
    Example:
        ...
        >>>     return data
    """
    translated_ptr = _translate_ptr(pointer, to_rank, from_rank, heap_bases, hint)
...
def store(
    ...
    """
    translated_ptr = _translate_ptr(pointer, from_rank, to_rank, heap_bases, hint)
...
def get(
    ...
    """
    translated_from_ptr = _translate_ptr(from_ptr, from_rank, to_rank, heap_bases, hint)
...
def put(
    ...
    """
    translated_to_ptr = _translate_ptr(to_ptr, from_rank, to_rank, heap_bases, hint)
